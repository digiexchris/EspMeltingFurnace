...
#define ESP_LCD_TOUCH_IO_SPI_XPT2046_CONFIG(touch_cs) \
	...
	/**
	 * @brief Set the mutex for SPI reads. Useful if this SPI bus is shared by more than one device.
	 *
	 * @param mutex: The mutex to use for SPI communication
	 * @return
	 *      - ESP_OK                    on success
	 *      - ESP_ERR_INVALID_ARG       if @param mutex is null.
	 */

	esp_err_t esp_lcd_touch_xpt2046_set_spi_mutex(SemaphoreHandle_t mutex);
